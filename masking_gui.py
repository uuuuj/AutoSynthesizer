...
def _mask_text(value: str, rng: np.random.Generator) -> str:
    ...
    # 일반 텍스트 — 같은 길이의 랜덤 문자열
    result = []
    for ch in s:
        if ch.isalpha():
            if '\uac00' <= ch <= '\ud7a3':  # 한글
                result.append(chr(rng.integers(0xAC00, 0xD7A4)))
            elif ch.isupper():
        ...
        try:
            ...
            for i in range(total):
                ...
                if pd.isna(val) or val is None:
                    ...
                    val_str = str(val)
                    if val_str not in masking_map:
                        masking_map[val_str] = mask_value(val_str, rng)
                    masked_values.append(masking_map[val_str])
            ...
            self.root.after(0, self._log,
                            f"[마스킹 완료] 고유값 {len(masking_map)}개 변환")
            ...
            key_data = {
                ...
                'total_rows': total,
                'unique_values': len(masking_map),
                'mapping': {v: k for k, v in masking_map.items()},  # 역방향: 마스킹값→원본값
            }
